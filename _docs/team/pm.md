...
- Read the task as written in `_docs/tasks.md`
...
- The tasks has all four sections filled in
- Every acceptance criterion can be checked by looking at the result
- Everything moved out of scope links to a follow-up task
...
File a follow-up task and list it under out of scope with a link to
that task, so it is clear what was moved and where it went.